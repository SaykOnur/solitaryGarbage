...
class SmoothFollow : MonoBehaviour
{
    ...
    [SerializeField] LayerMask layersToNotPassTrough;

    void LateUpdate()
    {
        Vector3 desiredPosition = pivot.position + offset;
        desiredPosition = Vector3.SmoothDamp(transform.position,desiredPosition, ref camVelocity,smoothSpeed,5f);

        if (Physics.Linecast(transform.position,desiredPosition,layersToNotPassTrough)) { desiredPosition = transform.position; }
